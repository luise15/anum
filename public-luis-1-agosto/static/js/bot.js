...
window.onload = function () {//crea el objeto ventana del navegadaros, es decir, un elemento que tiene contiene
    ...
    let n_plantas = document.getElementById('n_plantas');
    ...
    checkbox_modo1.onclick = function () {
        if ( checkbox_modo1.checked && (isEmpty(n_usuarios) || isEmpty(n_plantas)) ){
            block_modo1 = false;
            alert('Porfavor, ingresa una cantida de plantas valida');
        }
        ...
        if ( checkbox_modo1.checked && !block_modo1 && !isEmpty(n_usuarios) && !isEmpty(n_plantas) ){//si el checkbox esta con un check
    ...
    //--------------------------------------------------------------------------------------------------------------------------------------------
    //Este bloque de codigo controla el modo 2 de funcionamiento: Streaming de datos con maximo y mínimo
    ...
    checkbox.onclick = function () {
        if ( checkbox.checked && (isEmpty(ID_input) || isEmpty(diff_t_input) ||    // Esto dice que si el checkbox 'Comenzar streaming' no esta checado
            isEmpty(t_max_input) || isEmpty(t_min_input) || isEmpty(h_max_input) ||// y los compos estan vacios y si la temperatura máxima
